        ...
        for (String doc: documentSet.getDocuments()){
            switch (getParameterAsInt(PARAMETER_REMOVE_CONDITION)) {
                case CONDITION_MATCHES:
                    if (useRegex) {
                        conditionSatisfied = doc.matches(expr);
                    } else {
                        conditionSatisfied = doc.equals(expr);
                    }
                    break;
                case CONDITION_CONTAINS:
                    if (useRegex) {
                        conditionSatisfied = Pattern.compile(expr).matcher(doc).find();
                    } else {
                        conditionSatisfied = doc.contains(expr);
                    }
                    break;
            }
            if (conditionSatisfied == inverseCondition) {
                output.add(doc);
            }
        }
